...
# 定义模型参数
input_shape = (512, 512, 15)  # 根据你的数据调整输入形状（15 通道）
...
def calculate_miou(y_true, y_pred, num_classes=4):
    """
    Calculate mean Intersection over Union (mIoU)
    :param y_true: ground truth labels, shape (batch_size, height, width, num_classes)
    ...
    for c in range(num_classes):
        true_class = tf.equal(y_true, c)
        pred_class = tf.equal(y_pred, c)
        ...
        # if tf.equal(union, 0):
        #     iou = tf.constant(1.0)  # If there is no ground truth or predicted instance in this class
        # else:
        iou = intersection / union
        
        iou_list.append(iou)
    
    miou = tf.reduce_mean(tf.stack(iou_list))
    return miou
...
# 保存模型
model.save('inceptionv3_fcn_model_15_features_2.h5')
print("Model saved as inceptionv3_fcn_model_15_features_2.h5")
